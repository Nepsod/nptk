...
use nptk_core::vg::kurbo::{
    Affine, Circle, Line, Rect, RoundedRect, RoundedRectRadii, Shape, Stroke,
};
...
impl SecretInput {
    ...
    fn masked_text(&self) -> String {
        if self.show_password {
            ...
            self.mask_char
                .to_string()
                .repeat(self.buffer.text().chars().count())
        }
    }
...
impl Widget for SecretInput {
    fn render(
        ...
        // Get colors from palette (same as TextInput)
        ...
        let _text_color = palette.color(ColorRole::BaseText);
        ...
        if self.show_password {
            ...
            graphics.stroke(
                ...
                &Line::new(
                    (
                        toggle_center_x - slash_length / 2.0,
                        toggle_center_y - slash_length / 2.0,
                    ),
                    (
                        toggle_center_x + slash_length / 2.0,
                        toggle_center_y + slash_length / 2.0,
