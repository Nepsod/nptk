...
pub trait Signal<T: 'static>: 'static {
    ...
    /// Clones the signal into a boxed trait object.
...
impl<T: 'static> MaybeSignal<T> {
    ...
    /// Converts the [MaybeSignal] into a [BoxedSignal] if it is a [MaybeSignal::Signal].
...
impl<T: 'static> Clone for MaybeSignal<T> {
    fn clone(&self) -> Self {
        match self {
            ...
            MaybeSignal::Value(value) => MaybeSignal::Value(value.clone()),
        }
    }
}
