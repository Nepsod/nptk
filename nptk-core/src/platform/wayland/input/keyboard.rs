    ...
    fn event(
        ...
        match event {
            ...
            wl_keyboard::Event::Key {
                ...
                let focused = state.shared.get_focused_surface_key();
                log::debug!(
                    "Wayland keyboard key event: key={}, state={:?}, focused_surface={:?}",
                    key,
                    key_state,
                    focused
                );
                ...
                let target_surface = if let Some(key_surface) = focused {
                    ...
                    if let Some(surface_key) = first_surface_key {
                        log::debug!(
                            "No focused surface, using first registered surface {} as fallback",
                            surface_key
                        );
                        Some(surface_key)
                    } else {
            ...
            wl_keyboard::Event::Keymap { format, fd, size } => {
                log::debug!(
                    "Wayland keyboard keymap received: format={:?}, size={}",
                    format,
                    size
                );
