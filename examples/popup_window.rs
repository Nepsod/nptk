use nptk::core::layout::{FlexDirection, LayoutStyle, LengthPercentage, Rect};
...
impl Application for PopupExample {
    ...
    fn build(context: AppContext, _state: Self::State) -> impl Widget {
        ...
        Container::new(vec![])
            ...
            .with_child(Text::new("Main Window".to_string()).with_font_size(24.0))
            .with_child(
                Button::new(Text::new("Open Popup".to_string())).with_on_pressed(
                    MaybeSignal::signal(Box::new(EvalSignal::new(move || {
                        ...
                        let popup_content = Container::new(vec![])
                            ...
                            .with_child(
                                Text::new("I am a popup window!".to_string()).with_font_size(20.0),
                            );
