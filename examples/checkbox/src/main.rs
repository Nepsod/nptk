...
impl Application for MyApp {
    ...
    fn build(context: AppContext, _: Self::State) -> impl Widget {
        let checkbox_state = context.use_signal(StateSignal::new(CheckboxState::Unchecked));

        Container::new(vec![
            Box::new(Text::new("Simple Three-State Checkbox Test".to_string())),
            Box::new(Checkbox::new(MaybeSignal::signal(checkbox_state.clone()))),
            Box::new(Text::new(checkbox_state.map(|val| nptk::core::reference::Ref::Owned(format!("State: {:?}", *val))))),
        ])
