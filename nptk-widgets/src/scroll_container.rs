...
use nptk_core::layout::{LayoutNode, LayoutStyle, Dimension, StyleNode, LengthPercentageAuto, Layout};
use nptk_core::signal::{MaybeSignal, Signal, state::StateSignal, map::MapSignal};
...
impl ScrollContainer {
    ...
    fn get_vertical_scrollbar_bounds(&self, layout: &LayoutNode) -> Rect {
        ...
        let scrollbar_x = container_bounds.x1 - self.scrollbar_width as f64;
    ...
    fn get_horizontal_scrollbar_bounds(&self, layout: &LayoutNode) -> Rect {
        ...
        Rect::new(
            container_bounds.x0,
            scrollbar_y,
            container_bounds.x0 + scrollbar_width,
            container_bounds.y1,
        )
    }
    ...
    fn get_vertical_thumb_bounds(&self, scrollbar_bounds: Rect) -> Rect {
        if self.content_size.y <= self.viewport_size.y {
            return scrollbar_bounds;
        }

        let thumb_height = (self.viewport_size.y / self.content_size.y * scrollbar_bounds.height() as f32).max(20.0);
        let scroll_ratio = self.scroll_offset().y / (self.content_size.y - self.viewport_size.y);
        let thumb_y = scrollbar_bounds.y0 + scroll_ratio as f64 * (scrollbar_bounds.height() - thumb_height as f64);
    ...
    fn get_horizontal_thumb_bounds(&self, scrollbar_bounds: Rect) -> Rect {
        ...
        let thumb_width = (self.viewport_size.x / self.content_size.x * scrollbar_bounds.width() as f32).max(20.0);
        let scroll_ratio = self.scroll_offset().x / (self.content_size.x - self.viewport_size.x);
        let thumb_x = scrollbar_bounds.x0 + scroll_ratio as f64 * (scrollbar_bounds.width() - thumb_width as f64);
    ...
    fn render_scrollbar(&self, scene: &mut Scene, theme: Option<&nptk_theme::style::Style>, scrollbar_bounds: Rect, thumb_bounds: Rect, _is_vertical: bool, is_hovered: bool) {
...
impl Widget for ScrollContainer {
    ...
    fn render(&mut self, scene: &mut Scene, theme: &mut dyn Theme, layout: &LayoutNode, _info: &AppInfo, context: AppContext) -> () {
        ...
        let content_rect = Rect::new(
            layout.layout.location.x as f64,
            layout.layout.location.y as f64,
            (layout.layout.location.x + content_width) as f64,
            (layout.layout.location.y + content_height) as f64,
        );
        ...
        if let Some(child) = &mut self.child {
            ...
            // The layout system should handle positioning based on margins set in layout_style.
            child.render(scene, theme, base_layout, _info, context);
        ...
        if self.needs_vertical_scrollbar() {
            ...
            self.render_scrollbar(scene, widget_theme_ref, scrollbar_bounds, thumb_bounds, true, self.vertical_scrollbar_hovered);
        }
        ...
        if self.needs_horizontal_scrollbar() {
            ...
            self.render_scrollbar(scene, widget_theme_ref, scrollbar_bounds, thumb_bounds, false, self.horizontal_scrollbar_hovered);
        }
    }
    ...
    fn update(&mut self, layout: &LayoutNode, context: AppContext, info: &AppInfo) -> Update {
        ...
        if let Some(scroll_delta) = info.mouse_scroll_delta {
            ...
            // If scroll offset changed, trigger both layout and draw updates
    ...
    fn layout_style(&self) -> StyleNode {
        ...
        StyleNode {
            ...
            children: if let Some(child) = &self.child {
                // Apply scroll offset as reactive margins
                let mut child_style = child.layout_style();
                
                // Apply scroll offset as negative margins to shift content
                child_style.style.margin.left = LengthPercentageAuto::length(-scroll_offset.x);
                child_style.style.margin.top = LengthPercentageAuto::length(-scroll_offset.y);
